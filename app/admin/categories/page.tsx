...
import { categories as initialCategories, type Category } from '@/lib/products';
...
function CategoryModal({
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl2 animate-slide-up">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className={`h-20 w-full rounded-xl bg-gradient-to-br ${form.gradient} flex items-center justify-center transition-all duration-300`}>
            <span className="font-display text-lg font-bold text-stone-700 opacity-60">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor={`${uid}-name`} className="block text-xs font-semibold text-stone-500 mb-1.5">
            ...
            <div>
              <label htmlFor={`${uid}-slug`} className="block text-xs font-semibold text-stone-500 mb-1.5">
          ...
          {/* Description */}
          <div>
            <label htmlFor={`${uid}-desc`} className="block text-xs font-semibold text-stone-500 mb-1.5">
              Description
            </label>
            <input
              id={`${uid}-desc`}
          ...
          {/* Product count */}
          <div>
            <label htmlFor={`${uid}-count`} className="block text-xs font-semibold text-stone-500 mb-1.5">
              Product Count
            </label>
            <input
              id={`${uid}-count`}
...
function CategoryCard({
  ...
  return (
    <div className="group relative rounded-2xl border border-stone-200 bg-white p-5 shadow-card hover:shadow-card-hover transition-shadow">
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          {cat.description && (
            <p className="mt-1 text-xs text-stone-500 leading-relaxed line-clamp-2">{cat.description}</p>
          )}
        </div>
      ...
      {confirming && (
        <div className="absolute inset-0 rounded-2xl bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center gap-3 p-4 border-2 border-red-200 animate-fade-in">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-stone-500 mt-0.5 leading-relaxed">
              &ldquo;{cat.name}&rdquo; will be removed.
            </p>
          </div>
...
export default function AdminCategories() {
  ...
  const [modal, setModal]   = useState<{
    open: boolean; mode: 'add' | 'edit'; target: string | null;
  ...
  const openAdd  = () => setModal({ open: true, mode: 'add', target: null });
  const openEdit = (slug: string) => setModal({ open: true, mode: 'edit', target: slug });
  const closeModal = () => setModal({ open: false, mode: 'add', target: null });
  ...
  const handleSave = (data: typeof EMPTY_FORM) => {
    ...
    if (modal.mode === 'add') {
      if (cats.some((c) => c.slug === entry.slug)) {
        alert(`Slug "${entry.slug}" already exists. Choose a different name.`);
        return;
      }
      setCats((prev) => [...prev, entry]);
      showToast(`"${entry.name}" added`);
    } else {
      setCats((prev) => prev.map((c) => c.slug === modal.target ? entry : c));
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      {filtered.length === 0 && cats.length > 0 && (
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 z-[400] -translate-x-1/2 flex items-center gap-2 rounded-2xl bg-stone-900 px-5 py-3 text-sm font-medium text-white shadow-xl2 animate-slide-up">
