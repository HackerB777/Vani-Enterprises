      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-card space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor={`${uid}-name`} className="block text-xs font-semibold text-stone-500 mb-1.5">Product Name <span className="text-red-500">*</span></label>
              <input id={`${uid}-name`} required type="text" value={form.name} onChange={(e) => handleNameChange(e.target.value)} placeholder="e.g. Kanjivaram Silk Saree"
            ...
            <div>
              <label htmlFor={`${uid}-slug`} className="block text-xs font-semibold text-stone-500 mb-1.5">URL Slug <span className="text-red-500">*</span></label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-stone-300 font-mono select-none">/</span>
                <input id={`${uid}-slug`} required type="text" value={form.slug} onChange={(e) => { setSlugEdited(true); set('slug', e.target.value); }} placeholder="kanjivaram-silk-saree"
            ...
            <div>
              <label htmlFor={`${uid}-category`} className="block text-xs font-semibold text-stone-500 mb-1.5">Category <span className="text-red-500">*</span></label>
              <select id={`${uid}-category`} required value={form.category} onChange={(e) => set('category', e.target.value)}
            ...
            <div>
              <label htmlFor={`${uid}-subcategory`} className="block text-xs font-semibold text-stone-500 mb-1.5">Subcategory</label>
              <input id={`${uid}-subcategory`} type="text" value={form.subcategory} onChange={(e) => set('subcategory', e.target.value)} placeholder="e.g. Silk Sarees"
                className="w-full rounded-xl border border-stone-200 px-3.5 py-2.5 text-sm outline-none transition focus:border-brand-400 focus:ring-2 focus:ring-brand-100" />
            </div>
          </div>
          <div>
