...
function Avatar({ name, size = 'lg' }: { name: string; size?: 'sm' | 'lg' }) {
  ...
  const cls = size === 'lg'
    ...
    : 'h-9 w-9 text-sm';
  return (
    <div className={`${cls} flex shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-brand-600 to-brand-800 font-bold text-white shadow-inner`}>
      {initials}
    </div>
  );
}
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                >
                  <span className={`flex h-10 w-10 items-center justify-center rounded-full ${stat.color} text-xl`}>
                    {stat.icon}
                  </span>
          ...
          <div className="space-y-4">
            ...
            {ordersLoad ? (
              <div className="space-y-3">
                {[1,2,3].map((n) => <div key={n} className="h-20 animate-pulse rounded-xl bg-stone-200" />)}
              </div>
            ) : ordersErr ? (
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-stone-300 bg-white py-16 text-center">
                ...
                <Link href="/shop" className="mt-4 rounded-full bg-brand-600 px-6 py-2 text-sm font-semibold text-white hover:bg-brand-700 transition">
                  Start Shopping
                </Link>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold capitalize ${sc}`}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {wishlistItems.map((product) => (
                  <WishlistCard
                    key={product.slug}
                    product={product}
                    onRemove={() => removeWishlist(product.slug)}
        ...
        return (
          <div className="max-w-lg space-y-5">
            ...
            <form onSubmit={saveProfile} className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm space-y-5">
              ...
              <div>
                <label htmlFor="acc-name" className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-stone-500">
                  Full Name *
                </label>
                <input
                  id="acc-name"
              ...
              <div>
                <label htmlFor="acc-email" className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-stone-500">
                  Email Address
                </label>
                <input
                  id="acc-email"
              ...
              <div>
                <label htmlFor="acc-phone" className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-stone-500">
                  Mobile Number
                </label>
              ...
              <button
                type="submit"
                disabled={pSaving}
                className="w-full rounded-xl bg-brand-600 py-3 text-sm font-bold text-white transition hover:bg-brand-700 disabled:opacity-60 disabled:cursor-not-allowed"
              >
          ...
          <div className="max-w-lg space-y-5">
            ...
            <form onSubmit={changePassword} className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm space-y-5">
              ...
              {(['Current Password', 'New Password', 'Confirm New Password'] as const).map((label, i) => {
                ...
                return (
                  <div key={id}>
                    <label htmlFor={id} className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-stone-500">
                      {label}
                    </label>
                    <div className="flex rounded-xl border border-stone-200 bg-stone-50 overflow-hidden focus-within:border-brand-400 focus-within:bg-white transition">
                      <input
    ...
    <div className="flex flex-col overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm transition hover:shadow-md">
      ...
      <div className="flex flex-1 flex-col p-3 gap-2">
        ...
        <Link href={`/product/${product.slug}`}>
          <h3 className="text-sm font-semibold text-stone-800 line-clamp-2 leading-snug hover:text-brand-700 transition-colors">
            {product.name}
          </h3>
        </Link>

        {product.rating && (
          <span className="inline-flex w-fit items-center gap-0.5 rounded bg-green-600 px-1.5 py-0.5 text-[11px] font-bold text-white">
            {product.rating} ★
          </span>
        )}
