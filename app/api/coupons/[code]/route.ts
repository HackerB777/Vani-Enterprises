...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  if (data.validFrom  && new Date(data.validFrom)  > now) return NextResponse.json({ error: 'This coupon is not valid yet' }, { status: 400 });
  if (data.validUntil && new Date(data.validUntil) < now) return NextResponse.json({ error: 'This coupon has expired' }, { status: 400 });
